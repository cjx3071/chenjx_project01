<%@page import="com.zxx.beans.Feedback"%>
...
<title>意见反馈</title>
 ...
 <div class="Feedback_style">
  ...
  <div class="border clearfix">
       ...
       <span class="r_f">共：<b>${fn:length(feedbackList)}</b>条</span>
     </div>
    <div class="feedback">
	...
	<c:forEach items="${feedbackList}" var="feedback">
     ...
     <td><label><input type="checkbox" class="ace"><span class="lbl"></span></label></td>
          <td>${feedback.id}</td>
          <td>${feedback.f_uname }</td>
          <td>${feedback.f_uphone }</td>
          <td>${feedback.f_uemail }</td>
          <td class="text-l displayPart" displayLength="90">
          <a href="javascript:;" onclick="Guestbook_iew('12')">${feedback.mcontent }</a>
          <td>${feedback.ftime }</td>
          <td class="td-status"><span class="label label-success radius">${feedback.fstate }</span></td>
          <td>${feedback.fclass }</td>
          <td class="td-manage">    
        <a  href="javascript:;"  onclick="member_del(this,'${feedback.id}')" title="删除" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
...
function member_del(obj,id){
	var url="<%=basePath%>feedback/deleteFeedbackById";
