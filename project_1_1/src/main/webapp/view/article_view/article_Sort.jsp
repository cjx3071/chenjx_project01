<%@ page language="java" import="java.util.*,java.util.List,com.zxx.beans.Category" pageEncoding="UTF-8"%>
...
<div class="margin clearfix">
 <div class="sort_style">
  <div class="border clearfix">
       ...
       <span class="r_f">共：<b>${fn:length(categoryList)}</b>分类</span>
     </div>
     <!--分类类表-->
       ...
       <thead>
		...
		<c:forEach items="${categoryList}" var="category">
         <tr>
          ...
          <td>${category.id}</td>
          <td>${category.csort}</td>
          <td>${category.cname}</td>
          <td class="displayPart" displayLength="60">${category.cintfo}</td>
          <td>${category.ctime}</td>
          <td>${category.cstate}</td>          
          <td class="td-manage">   
           <a title="编辑" onclick="member_edit('510')" href="javascript:;"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>      
           <a title="删除" href="javascript:;"  onclick="member_del(this,'${category.id}')" class="btn btn-xs btn-danger" ><i class="fa fa-trash  bigger-120"></i></a>
          </td>
         </tr>
          </c:forEach>
        </tbody>
...
function member_del(obj,id){
	var url="<%=basePath%>category/deleteCategoryById";
