   ...
   <div class="Statistics_style" id="Statistic_pie">
	...
	<c:forEach var="user" items="${userList}">
		<tr>
          ...
          <td>${user.uid}</td>
          <td><u style="cursor:pointer" class="text-primary" onclick="member_show('${user.uname}','selectUserById','${user.uid}','500','400')">${user.uname}</u></td>
          <td>${user.usex}</td>
          <td>${user.uphone}</td>
          <td>${user.uemail}</td>         
          <td>${user.utime}</td>
          <td>${user.ugrade}</td>
          <td class="text-l">${user.uintegral}</td>
          <td class="td-status"><span class="label label-success radius">${user.ustate}</span></td>
          <td class="td-manage">
