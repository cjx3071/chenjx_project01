 ...
 <!--列表样式-->
	...
	<tbody>
      <c:forEach items="${advList}" var="adv">
       ...
       <td><span class="ad_img"><a href="products/ad1.jpg" data-rel="colorbox" data-title="广告图"><img src="products/ad1.jpg"  width="80%" height="80%"/></a></span></td>      
       <td>${adv.aLength}x${adv.aWidth}</td>
       <td><a href="#" target="_blank">${adv.aUrl}</a></td>
       <td>${adv.aTime}</td>
       <td class="td-status"><span class="label label-success radius">显示</span></td>
      <td class="td-manage">
         <a onClick="member_stop(this,'10001')"  href="javascript:;" title="停用"  class="btn btn-xs btn-success"><i class="fa fa-check  bigger-120"></i></a>    
        <a title="编辑" onclick="adv_edit(this,${adv.id})" href="javascript:;"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>      
        <a title="删除" href="javascript:;"  onclick="adv_del(this,${adv.id})" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
       </td>
      </tr>
      </c:forEach>
...
<!--添加广告样式-->
<div id="add_ads_style"  style="display:none" class="add_adv">
 <div class="add_adverts">
  <form action="<%=basePath%>adv/addAdv" method="post">
			<ul>
				<li><label class="label_name">图片尺寸</label><span
					class="cont_style"> <input name="aLength" type="text"
						id="form-field-1" placeholder="0" class="col-xs-10 col-sm-5"
						style="width:80px"> <span class="l_f"
						style="margin-left:10px;">x</span><input name="aWidth" type="text"
						id="form-field-1" placeholder="0" class="col-xs-10 col-sm-5"
						style="width:80px"></span></li>
				<li><label class="label_name">ID</label><span
					class="cont_style"><input name="aId" type="text"
						id="form-field-1" placeholder="0" class="col-xs-10 col-sm-5"
						style="width:50px"></span></li>
						<li><label class="label_name">分类</label><span
					class="cont_style"><input name="aClass" type="text"
						id="form-field-1" placeholder="0" class="col-xs-10 col-sm-5"
						style="width:50px"></span></li>
				<li><label class="label_name">链接地址</label><span
					class="cont_style"><input name="aUrl" type="text"
...
function adv_edit(obj,id) {
	layer.open({
		...
		yes : function(index, layero) {
			var num = 0;
			var str = "";
			var url = "<%=basePath%>adv/updateAdv?id="+id;
			$(".add_adv input[type$='text']").each(
					function(n) {
						if ($(this).val() == "") {
			...
			$(".add_adv input[type='radio']:checked").each(
					function(n) {
						url +=$(this).attr("name")+"="+$(this).val()+"&";
						alert(url);
					});
...
function adv_del(obj, id) {
	layer.confirm('确认要删除吗？', function(index) {
		...
		location.href="<%=basePath%>adv/deleteAdv?id="+id;
	});
}
jQuery(function($) {
